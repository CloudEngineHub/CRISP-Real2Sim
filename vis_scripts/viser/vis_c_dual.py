...
def main() -> None:
    ...
    for spec, scene_path, handle_prefix in [
        ...
        viser.add_mesh_simple(handle_name, verts, faces, color=(0.6, 0.7, 0.9))
    ...
    checkbox_handles: Dict[str, object] = {}
    visibility_state: Dict[str, bool] = {
        label: idx == 0 for idx, label in enumerate(label_order)
    }
    ...
    with server.gui.add_folder("Display"):
        for label in label_order:
            checkbox_handles[label] = server.gui.add_checkbox(
                f"Show {display_labels[label]}",
                visibility_state[label],
            )

    for label, checkbox in checkbox_handles.items():
        @checkbox.on_update
        def _(_event, label=label, checkbox=checkbox) -> None:
            visibility_state[label] = bool(checkbox.value)
            apply_visibility_state()
