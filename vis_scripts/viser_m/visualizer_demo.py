...
def main(
   ...
   if False:
    ...
    import yaml 
    urdf = '/data3/zihanwa3/_Robotics/humanoid/hybrid-imitation-parkour/post_asset_vmm/56_outdoor_stairs_up_down/robot_vis_info.yaml'
    info = yaml.safe_load(urdf)
    body_names = info.get("body_names", [])
    dt = float(info.get("dt", 1.0 / 60.0)) / max(args.speed, 1e-6)
    asset_xml_rel = info.get("asset_xml", None)
    npz_path = '/data3/zihanwa3/_Robotics/humanoid/hybrid-imitation-parkour/post_asset_vmm/56_outdoor_stairs_up_down/rigid_bodies_0.npz'
   ...
   hmr_types = ['postrl_vmm']# , 'gv'] # , 'postrl' tram 
   ...
   if True:
    ...
    if True:  # Set to True to enable rendering
      ...
      render_output_dir = f"/data3/zihanwa3/_Robotics/_geo/differentiable-blocksworld/rendered_views/vmm"
      os.makedirs(render_output_dir, exist_ok=True)

      render_output_dir = f"/data3/zihanwa3/_Robotics/_geo/differentiable-blocksworld/rendered_views/vmm/{tgt_name}"
      os.makedirs(render_output_dir, exist_ok=True)
      rendered_frames = []
