...
def compute_foot_sliding(target_verts, pred_verts, masks, thr=1e-2):
    ...
    foot_loc = target_verts[masks][:, foot_idxs]
    foot_disp = (foot_loc[1:] - foot_loc[:-1]).norm(2, dim=-1)
    contact = foot_disp[:] < thr
    pred_feet_loc = pred_verts[masks][:, foot_idxs]
...
def global_align_joints(gt_joints, pred_joints):
    ...
    return pred_glob, s_glob, R_glob, t_glob


def first_align_joints(gt_joints, pred_joints, fixed_scale=True, no_align=False, debug=False):
    """
    align the first two frames
    :param gt_joints (T, J, 3)
    :param pred_joints (T, J, 3)
    :param fixed_scale: if True, do not allow scaling
    :param no_align: if True, skip alignment and return original predictions
    :param debug: if True, return transformation parameters
    """
    
    if no_align:
        # Return original predictions without any alignment
        # Identity transformation: scale=1, rotation=I, translation=0
        s_first = torch.ones(1, 1, device=pred_joints.device, dtype=pred_joints.dtype)
        R_first = torch.eye(3, device=pred_joints.device, dtype=pred_joints.dtype).unsqueeze(0)
        t_first = torch.zeros(1, 3, device=pred_joints.device, dtype=pred_joints.dtype)
        
        # pred_first is just the original pred_joints
        pred_first = pred_joints
        
        if debug:
            return pred_first, s_first, R_first, t_first
        return pred_first
...
def align_pcl(Y, X, weight=None, fixed_scale=True):
