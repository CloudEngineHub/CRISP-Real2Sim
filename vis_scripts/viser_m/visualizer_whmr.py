...
def collect_contact_points_from_body(
    vertices: np.ndarray,
    part_ids_list: list[np.ndarray],
    scene_mesh: Optional[trimesh.Trimesh],
    distance_threshold: float,
    max_points_per_part: int = 12000,
    max_total_points: int = 60000,
) -> list[np.ndarray]:
    """Aggregate candidate contact points from body vertices that lie close to the scene mesh.

    Args:
        vertices: Array of body vertices with shape [T, V, 3] or [V, 3].
        part_ids_list: Vertex indices per body part.
        scene_mesh: Scene geometry in world coordinates.
        distance_threshold: Maximum distance (m) to treat a vertex as contacting the scene.
        max_points_per_part: Optional cap per body part after deduplication.
        max_total_points: Optional cap across all parts after deduplication.

    Returns:
        List of point clouds, one per body part.
    """
    if vertices is None or not part_ids_list:
        return [np.empty((0, 3), dtype=np.float32) for _ in part_ids_list]

    if isinstance(vertices, torch.Tensor):
        vertices = vertices.detach().cpu().numpy()

    if vertices.ndim == 2:  # single frame
    ...
    if scene_vertices.size == 0:
        return [np.empty((0, 3), dtype=np.float32) for _ in part_ids_list]

    tree = cKDTree(scene_vertices)
    rng = np.random.default_rng(42)
    part_point_sets: list[np.ndarray] = []

    for part_ids in part_ids_list:
        if part_ids.size == 0:
            part_point_sets.append(np.empty((0, 3), dtype=np.float32))
            continue

        part_vertices = vertices[:, part_ids, :].reshape(-1, 3)
        if part_vertices.size == 0:
            part_point_sets.append(np.empty((0, 3), dtype=np.float32))
            continue

        distances, _ = tree.query(part_vertices)
        close_points = part_vertices[distances <= distance_threshold]
        if close_points.size == 0:
            part_point_sets.append(np.empty((0, 3), dtype=np.float32))
            continue

        close_points = close_points.astype(np.float32)
        close_points = np.unique(np.round(close_points, decimals=4), axis=0)

        if max_points_per_part and close_points.shape[0] > max_points_per_part:
            idx = rng.choice(close_points.shape[0], size=max_points_per_part, replace=False)
            close_points = close_points[idx]

        part_point_sets.append(close_points)

    if max_total_points:
        total = sum(points.shape[0] for points in part_point_sets)
        if total > max_total_points:
            scale = max_total_points / total
            for i, points in enumerate(part_point_sets):
                count = points.shape[0]
                if count <= 1:
                    continue
                target = max(1, int(np.floor(count * scale)))
                target = min(target, count)
                if target < count:
                    idx = rng.choice(count, size=target, replace=False)
                    part_point_sets[i] = points[idx]

    return part_point_sets
...
def main(
   ...
   hmr_types = ['GT', 'videomicmic', 'postrl_vmm', 'postrl_ours', 'tram']# , 'gv'] # , 'postrl' tram 
   ...
   for hmr_type in hmr_types:
       ...
       if hmr_type == 'tram':
           ...
           gt_ori, gt_j3d, gt_vert, valid, seq_len, start_idx = read_gt_EMDB2(gt_hmr, raw_clips_path)
           gt_j3d = gt_j3d.cpu().numpy()
           gt_vert = gt_vert.cpu().numpy()

           pred_vert = gt_vert
           pred_j3d = gt_j3d
           num_frames=len(gt_vert)

       elif hmr_type == 'videomicmic':
          ...
          if undo:
            ...
            uncalibrated_rotmat = pred_rotmat.clone()
            uncalibrated_trans = pred_trans.squeeze()
          ...
          pred_j3d = pred.joints[:, :24].numpy()
           ...
           # faces = smpl_results['faces']# .cpu().numpy()
           num_frames=smpl_results['num_frames']
           pred_j3d =pred_j3dg = smpl_results['pred_j3dg']

       elif hmr_type == 'postrl_ours':
            rl_path = '/data3/zihanwa3/_Robotics/humanoid/hybrid-imitation-parkour/post_results_ours'
            my_hmr = os.path.join(rl_path, f'{tgt_name}_afterrl_rotback.npz')
            pred_smpl = np.load(my_hmr)# .item()
            poses = tt(pred_smpl['poses'])
            betas = tt(pred_smpl['betas'])
            transl_orig = tt(pred_smpl['trans'])
            #poses = matrix_to_axis_angle_pytorch3d(poses)
            #global_orient = matrix_to_axis_angle_pytorch3d(global_orient)
            # gt_hmr = find_gt_file(tgt_name, args.gt_data_dir)
            output = smpl_template(betas=torch.zeros_like(betas), # T, 10
                                  transl = transl_orig, # T, 3
                                  global_orient = poses[:,:3], # T, 3
                                  body_pose = poses[:,3:], # T, 69
                                  return_verts=True)
            # print(dir(output))
            
            pred_vert = output.vertices.detach().cpu().numpy()
            pred_j3d = output.joints[:, :24].numpy()

       elif hmr_type == 'postrl_vmm':
            rl_path = '/data3/zihanwa3/_Robotics/humanoid/hybrid-imitation-parkour/post_results_vmm'
            my_hmr = os.path.join(rl_path, f'{tgt_name}_afterrl_rotback.npz')
            pred_smpl = np.load(my_hmr)# .item()
            poses = tt(pred_smpl['poses'])
            betas = tt(pred_smpl['betas'])
       ...
       if ALIGN and hmr_type != 'GT':
          ...
          #w_j3d, s_first, R_first, t_first = first_align_joints(gt_j3d[:len(pred_j3d)], pred_j3d)#.cpu().numpy()
          #print(s_first.shape, R_first.shape, t_first.shape)
          #R_first = R_first[0]
          #t_first = t_first[0]



          print(pred_j3d, pred_j3d, hmr_type)
       ...
       pred_vert = np.asarray(pred_vert, dtype=np.float32)
   ...
   body_part_names = []
   part_ids_list = []
   contact_color_arrays = []
   contact_distance_threshold = 0.03  # meters
   contact_max_points_per_part = 12000
   contact_max_total_points = 60000
   empty_contact_template = []
   ...
   if True:
        ...
        body_part_configs = [
            ("leg",     np.asarray(leg_ids_smpl, dtype=np.int64),     0.95, np.array([0, 255,   0], dtype=np.uint8)),
            ("hand",    np.asarray(hand_ids_smpl, dtype=np.int64),    0.95, np.array([255,   0,   0], dtype=np.uint8)),
            ("gluteus", np.asarray(gluteus_ids_smpl, dtype=np.int64), 0.90, np.array([255, 255,   0], dtype=np.uint8)),
            ("back",    np.asarray(back_ids_smpl, dtype=np.int64),    0.90, np.array([  0,   0, 255], dtype=np.uint8)),
            ("thigh",   np.asarray(thigh_ids_smpl, dtype=np.int64),   0.95, np.array([255,   0, 255], dtype=np.uint8)),
        ]

        body_part_names = [cfg[0] for cfg in body_part_configs]
        part_ids_list = [cfg[1] for cfg in body_part_configs]
        contact_color_arrays = [cfg[3] for cfg in body_part_configs]
        empty_contact_template = [np.empty((0, 3), dtype=np.float32) for _ in part_ids_list]
   ...
   hmr_mesh_handles = {}
   postrl_contact_handles: dict[str, list[viser.PointHandle]] = {}
        ...
        if gui_show_all_frames.value:
            ...
            with server.atomic():
                ...
                for ht, handles in hmr_mesh_handles.items():
                    ...
                    if ht in postrl_contact_handles:
                        contact_visible = show_ht and gui_show_points.value
                        for handle in postrl_contact_handles[ht]:
                            handle.visible = contact_visible
        else:
   ...
   for hmr_type in hmr_types:
       ...
       def _(_, ht=hmr_type):
           ...
           if ht in postrl_contact_handles:
               contact_visible = gui_hmr_controls[ht].value and gui_show_points.value
               for handle in postrl_contact_handles[ht]:
                   handle.visible = contact_visible
   ...
   def _(_):
       ...
       for ht, handles in postrl_contact_handles.items():
           show_ht = gui_hmr_controls.get(ht)
           want_visible = gui_show_points.value and (show_ht.value if show_ht is not None else True)
           for handle in handles:
               handle.visible = want_visible
   ...
   if True:
    sqs_mesh_path = os.path.join('/data3/zihanwa3/_Robotics/_vision/mega-sam/post_results', tgt_name, 'scene_mesh_nksr', 'scene_nksr_coacd_mesh_nksr.obj')#'scene_mesh_sqs', 'scene_mesh_sqs.obj')
    sqs_mesh_original = trimesh.load(sqs_mesh_path)

    # 假设你已经有 s_first, R_first, t_first

    try:
      A = make_affine(s_first_, R_first_, t_first_)
   ...
   for hmr_type, hmr_result in hmr_results.items():
        ...
        contact_points_by_part = hmr_result.get('contact_points_by_part')
        if hmr_type.startswith('postrl') and part_ids_list:
            if scene_mesh_contact_tf is not None:
                contact_points_by_part = collect_contact_points_from_body(
                    pred_vert,
                    part_ids_list,
                    scene_mesh_contact_tf,
                    distance_threshold=contact_distance_threshold,
                    max_points_per_part=contact_max_points_per_part,
                    max_total_points=contact_max_total_points,
                )
            else:
                contact_points_by_part = [arr.copy() for arr in empty_contact_template]
            hmr_result['contact_points_by_part'] = contact_points_by_part
        ...
        contact_handles = []
        contact_points_by_part = hmr_result.get('contact_points_by_part')
        if contact_points_by_part:
            for part_name, points, color_arr in zip(body_part_names, contact_points_by_part, contact_color_arrays):
                points_np = np.asarray(points, dtype=np.float32)
                if points_np.size == 0:
                    continue
                colors_np = np.repeat(color_arr[None, :], points_np.shape[0], axis=0)
                handle = server.scene.add_point_cloud(
                    name=f"/contact/{hmr_type}/{part_name}",
                    points=points_np,
                    colors=colors_np,
                    point_size=max(point_size, 0.008),
                    point_shape="rounded",
                )
                handle.visible = True
                contact_handles.append(handle)

        if contact_handles:
            postrl_contact_handles[hmr_type] = contact_handles
   ...
   # ADDED: Get HMR vertices for contact detection (using first HMR type available)
   first_hmr_type = hmr_types[0] if hmr_types else None
   hmr_vertices_all = None
   if first_hmr_type and first_hmr_type in hmr_results:
       hmr_vertices_all = hmr_results[first_hmr_type]['pred_vert']# .cpu().numpy()
   ...
   if False:
    group_names = ["leg", "hand", "gluteus", "back", "thigh"]
    contact_groups_pts = {name: arr for name, arr in zip(group_names, pred_contact_vert_list)}
    colors_rgb = {name: color for name, color in zip(group_names, contact_colors_rgb)}

    # 2) Choose a realistic radius (meters)
    radius_m = 0.1  # 3 cm

    # 3) Create one colored overlay mesh for ALL frames
    colored_contact_mesh = colorize_scene_by_multiframe_contacts(
        nksr_mesh_original,
        contact_groups_pts=contact_groups_pts,
        colors_rgb=colors_rgb,
        radius=radius_m,
    )

    # 4) Show it once; hide the base when overlay is on (to avoid z-fighting)
    overlay_handle = server.scene.add_mesh_trimesh("/contact_all_frames_overlay", colored_contact_mesh)
    overlay_handle.visible = True
    if nksr_mesh_handle:
        nksr_mesh_handle.visible = False
    # Add colored mesh to scene
    contact_handle = server.scene.add_mesh_trimesh(
        f"/frames/t0/contacted_mesh",
        mesh=colored_contact_mesh,
    )
    contact_handle.visible = False  # Initially hidden
    contact_mesh_handles.append(contact_handle)
   ...
   # Set initial visibility for all frame nodes
   for i, frame_node in enumerate(frame_nodes):
       if gui_show_all_frames.value:
