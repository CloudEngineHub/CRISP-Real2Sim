...
def export_scene_as_urdf(parts, transforms, out_dir, postfix):
    ...
    out_dir = Path(out_dir) / f'pieces{postfix}'
...
def main(
  ...
  thick: bool = False,
    ...
    input_base_file = f'/data3/zihanwa3/_Robotics/_vision/mega-sam/post_results/{tgt_name}/{hmr_type}/scene'
    #if os.path.exists(input_base_file):
    #    if os.path.isfile(input_base_file):
    #        os.remove(input_base_file)          # remove file
    #    else:
    #         shutil.rmtree(input_base_file)      # remove directory


    input_file = f"{input_base_file}/scene_mesh.obj"
    ...
    if merge: 
        ...
        prefix = ''
    nksr_coacd_mesh = input_file.replace('scene_mesh', f'{prefix}scene_vdb_coacd_mesh')
    ...
    '''
    input_base_file = f'post_results/{tgt_name}/{hmr_type}/scene'
    input_file = f"{input_base_file}/scene_mesh.obj"
    '''
    nksr_coacd_mesh = input_file.replace('scene_mesh', f'{prefix}scene_vdb_coacd_mesh')
    scene.export(nksr_coacd_mesh)
    ...
    if move:
        ...
        BIGBIG_folder_tgt = os.path.join(BIGBIG_folder_tgt, hmr_type,'scene')
