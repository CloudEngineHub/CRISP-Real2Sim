...
CAN_COLLECT_CONTACTS = CONTACT_UTILS_AVAILABLE and select_frames_and_collect_contacts is not None
...
def main(
    ...
    def _(_) -> None:
        ...
        if not gui_show_all_frames.value:
            with server.atomic():
                ...
                if gui_show_contact.value and len(contact_vertices_handles) > 0:
                    # Hide all contact vertices
                    for handle in contact_vertices_handles:
                        handle.visible = False
                    
                    # Show contact vertices for current frame if available
    ...
    USE_CONTACT = True
    if 'qitao' in tgt_name or 'pk' in tgt_name:
      single_image = False 
      USE_CONTACT=False
    ...
    debug = False 
    contact_pipeline_available = CAN_RUN_CONTACT_ANALYSIS and CAN_COLLECT_CONTACTS
    ...
    if USE_CONTACT and contact_pipeline_available: 
        if not transfer_data:
          # os.path.join('/data3/zihanwa3/_Robotics/_geo/differentiable-blocksworld/post_results', tgt_name)
            ...
            body_part_params={
                    "leg":     {"contact_threshold": 0.95, "min_consecutive_frames": 45, "weight": 1.0, "vel_threshold": 0.010, "min_lowvel_run": 5},
                    "hand":    {"contact_threshold": 0.95, "min_consecutive_frames": 15, "weight": 0.6, "vel_threshold": 0.008, "min_lowvel_run": 4},
                    "gluteus": {"contact_threshold": 0.95, "min_consecutive_frames": 90, "weight": 1.2, "vel_threshold": 0.014, "min_lowvel_run": 5},
                    "back":    {"contact_threshold": 0.90, "min_consecutive_frames": 60, "weight": 0.8, "vel_threshold": 0.010, "min_lowvel_run": 5},
                    "thigh":   {"contact_threshold": 0.95, "min_consecutive_frames": 90, "weight": 1.0, "vel_threshold": 0.014, "min_lowvel_run": 5},
                }
            contacted_masks, static_frames, static_segments, best_frames_global, counts_global, per_part = analyze_contacts_5parts(
                ...
                enable_vertex_smoothing=True,      # Smooth HMR poses
                vertex_smooth_window=7,            # Temporal smoothing window
                enable_contact_smoothing=True,     # Smooth contact scores
                contact_smooth_window=5,           # Contact smoothing window
                velocity_outlier_threshold=2.5,    # Remove velocity outliers
                velocity_smooth_window=9,           # Velocity smoothing
                debug_dir=os.path.join('/data3/zihanwa3/_Robotics/_geo/differentiable-blocksworld/post_results', tgt_name, 'contact_debug')
            )


            selected_frames, per_frame_pcs, all_points = select_frames_and_collect_contacts(
                ...
                pred_vert=pred_vert,              # [T, V, 3]
                policy="velocity",                  # or "velocity"
                body_part_params=body_part_params # optional; helps derive thresholds
            )

            contact_points = human_scene_contacing_pc = all_points
            print('!!! SELECTED: ', selected_frames, all_points.shape)
            contact_pc_filtered = server.scene.add_point_cloud(
                                    name=f"/frames/t0/point_cloud_filtered_contact",
            ...
            save_point_cloud_ply(
                path=f"//data3/zihanwa3/_Robotics/_geo/differentiable-blocksworld/cache_contact/{tgt_name}_contact_points.ply",
