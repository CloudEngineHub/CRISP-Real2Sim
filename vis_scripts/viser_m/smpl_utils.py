...
def load_contact_ids_from_file(filepath: str = '/data3/zihanwa3/_Robotics/_vision/mega-sam/CONTACT_IDS_SMPL.pt') -> torch.Tensor:
    """
...
def process_tram_smpl(
    ...
    results_root = Path("/data3/zihanwa3/_Robotics/_vision/tram/results")
...
def process_gv_smpl(
    ...
    hmr4d_path = f'/data3/zihanwa3/_Robotics/_vision/GVHMR/outputs/demo/{tgt_name}/hmr4d_results.pt'
    pred = torch.load(hmr4d_path)
    
    PROJ_ROOT = '/data3/zihanwa3/_Robotics/_vision/GVHMR/'
    smplx2smpl = torch.load(f"{PROJ_ROOT}/hmr4d/utils/body_model/smplx2smpl_sparse.pt").to(device)
    ...
    modelggg = BodyModelSMPLX(model_path=f"{PROJ_ROOT}/inputs/checkpoints/body_models", **bm_kwargs)
    ...
    bm_kwargs_smpl = {
        "model_path": f"{PROJ_ROOT}/inputs/checkpoints/body_models",
        "model_type": "smpl",
        "gender": "neutral",
        "num_betas": 10,
