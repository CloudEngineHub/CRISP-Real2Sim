...
def save_custom_mesh(mesh_parts, tgt_folder, postfix=''):
    scene = trimesh.Scene()
    keep_T = []
    for p in mesh_parts:
    ...
    export_scene_as_urdf(mesh_parts, keep_T, tgt_folder, postfix=postfix)

    
def export_scene_as_urdf(parts, transforms, out_dir, postfix):
    """
    ...
    out_dir = out_dir / f'pieces{postfix}'
    out_dir.mkdir(parents=True, exist_ok=True)
    ...
    ET.ElementTree(robot).write(
        urdf_path / f"{scene_type}{postfix}.urdf",
        encoding="utf-8",
        xml_declaration=True,
    )
    print(f'write as {scene_type}{postfix}.urdf')
...
def baseline_nksr(
    ...
    mesh = field.extract_dual_mesh(mise_iter=mise_iter, max_points=2 ** 17)
    return mesh
...
def main(
  ...
  thick: bool = False,
    ...
    device = 'cuda'
    try:
      bg_positions, normals = load_bg_normals(tgt_name, device=device, out_dir="cache", fmt="pt")
    except:
      return
    # 体素下采样到 ~2cm 网格，并再限制最多 2M 个点（按需修改/关闭）
    pts, nor = prepare_points_normals(
        bg_positions, normals, device=device,
        voxel_size=0.02,          # 关掉体素下采样就设为 None
        voxel_method="first",     # 可选: "first" | "random" | "mean"
        max_points=200_000_000,     # 关掉最大点限制就设为 None
        seed=0,
    )
    # hps_track
    input_base_file = f'out/{tgt_name}'
    input_base_file = f'outest_true/{tgt_name}'
    input_base_file = f'outest_Sep10/{tgt_name}'

    os.makedirs(input_base_file, exist_ok=True)
    input_file = f"{input_base_file}/scene_mesh_nksr.obj"
    nksr_mesh = baseline_nksr(pts, nor, device=device, detail_level=1.0, mise_iter=1)
    export_trimesh(nksr_mesh, input_file)  # 或 .obj / .glb
    nksr_path = input_file
    ...
    result = coacd.run_coacd(mesh,
                                ...
                                resolution=50000, #resolution=200000
                                #esolution=50000
                                )
    scene = trimesh.Scene()
    mesh_parts = []
    ...
    if merge: 
        ...
        prefix = ''
    nksr_coacd_mesh = input_file.replace('scene_mesh', f'{prefix}scene_nksr_coacd_mesh')
    scene.export(nksr_coacd_mesh)

    save_custom_mesh(mesh_parts, input_base_file)
    ...
    nksr_coacd_mesh = input_file.replace('scene_mesh', f'{prefix}scene_nksr_coacd_mesh')
    scene.export(nksr_coacd_mesh)
    save_custom_mesh(mesh_parts, input_base_file, postfix='_mergeFalse')



    # input_base_file
    megasam_out_folder = '/data3/zihanwa3/_Robotics/_vision/mega-sam/post_results'
    move=True
    if move:
        BIGBIG_folder_tgt = os.path.join(megasam_out_folder, tgt_name)
        BIGBIG_folder_tgt = os.path.join(BIGBIG_folder_tgt, 'scene_mesh_nksr')
        if os.path.exists(BIGBIG_folder_tgt):
            shutil.rmtree(BIGBIG_folder_tgt)     # ⚠️ 会把目标目录全部删除
        shutil.copytree(input_base_file, BIGBIG_folder_tgt)

    # os.makedirs(BIGBIG_folder_tgt, exist_ok=True)
    ...
    # BIGBIG_folder_tgt = input_base_file + folder 

    #nksr_output_path = os.path.join(BIGBIG_folder_tgt, 'scene_mesh_nksr.obj')
    # nksr_coacd_output_path = os.path.join(BIGBIG_folder_tgt, f'{prefix}_scene_mesh_nksr_coacd.obj')


    # shutil.copy(nksr_path, nksr_output_path)
    # shutil.copy(nksr_coacd_mesh, nksr_coacd_output_path)



    '''if thick:
      nksr_thicken_mesh = thicken_along_normals(nksr_mesh)
      input_file = f"out/scene_mesh_{tgt_name}.obj".replace(tgt_name, f'{tgt_name}_thicken')
      export_trimesh(nksr_thicken_mesh, input_file)  # 或 .obj / .glb
      mesh = trimesh.load(input_file, force="mesh")
      mesh = coacd.Mesh(mesh.vertices, mesh.faces)
      result = coacd.run_coacd(mesh,
                                  merge=False,
                                  ...
                                  # preprocess_resolution=80, #  50 by d
                                  # max_ch_vertex=256, # max_ch_vertex=512  default = 256
                                  resolution=2000000, #resolution=200000
                                  )
      ...
      for p in mesh_parts:
          #p.visual.vertex_colors[:, :3] = (np.random.rand(3) * 255).astype(np.uint8)
          scene.add_geometry(p)
      scene.export(input_file.replace('scene_mesh', 'scene_coacd_mesh'))'''
