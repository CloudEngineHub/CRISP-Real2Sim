...
def main(
   ...
   hmr_types = ['postrl_ours']# , 'gv'] # , 'postrl' tram 
       ...
       if hmr_type == 'tram':
          ...
          pred_j3d = pred.joints[:, :24].numpy()
           ...
           pred_j3d =pred_j3dg = smpl_results['pred_j3dg']
            ...
            rl_path = '/data3/zihanwa3/_Robotics/humanoid/hybrid-imitation-parkour/post_results_0917'
            ...
            rl_path = '/data3/zihanwa3/_Robotics/humanoid/hybrid-imitation-parkour/post_results'
            my_hmr = os.path.join(rl_path, f'{tgt_name}_afterrl_rotback.npz')
            pred_smpl = np.load(my_hmr)# .item()
            poses = tt(pred_smpl['poses'])
            betas = tt(pred_smpl['betas'])
   ...
   if True:
    base_papppath = f'/data3/zihanwa3/_Robotics/_geo/differentiable-blocksworld/__contact_ablate_geo/{tgt_name}/gv'
    sqs_mesh_path = nksr_mesh_path = os.path.join(base_papppath, 'scene_mesh_sqs_no_contact/scene_mesh_sqs.obj')
   ...
   T_world_cameras = []
   if True:
    # —— 放在采集循环之前，准备 per-frame 容器 ——
    frame_indices = []
    per_pts   = []   # 每帧的 points_bg_map  (Ni,3)
    per_cols  = []   # 每帧的 bg_color      (Ni,3) 或 float[0..1]
    per_Twc   = []   # 每帧的 cam_c2w       (4,4)
    per_K     = []   # 每帧的内参 K         (3,3)
    per_HW    = []   # 每帧的分辨率 (H,W)

    # —— 采集循环（保留你现有的 get_point_cloud / get_sdf，只换存储逻辑）——
    for i in tqdm(range(0, num_frames, interval)):
        ...
        depth, rotation, translation, K_img, points_bg_map, _ = extras

        # 建议从 get_sdf 返回里拿同一帧使用的 T_wc 与 K（若你那边 K 已一致也可以只用 K_img）
        _, T_wc, K_this = frame.get_sdf(bg_position, downsample_factor, bg_downsample_factor, vdb_volume=None)

        frame_indices.append(i)
        per_pts.append(po_all)
        per_cols.append(clr_all)
        per_Twc.append(T_wc)
        per_K.append(K_this.astype(np.float64))
        per_HW.append((H, W))
   ...
   if True:
    ...
    if True:  # Set to True to enable rendering
      # ========= 直接替换你现有的渲染块（从“Starting to render ...”开始那一段） =========

      # ========= Viser-based headless rendering (drop-in) =========
      print("Starting to render combined geometry to training views...")

      import time, imageio.v3 as iio
      from contextlib import suppress

      client, _browser_handles = ensure_headless_client(server)

      # --- 1) 准备/确保有一个前端客户端连接（headless） ---
      host = server.get_host()
      port = server.get_port()
      vis_url = f"http://{host}:{port}"
      print(f"[viser] expecting a client at {vis_url}")

      # 尝试自动拉起 headless Chromium（若已安装 playwright）
      ...
      with suppress(Exception):
          try:
              ...
              page = browser.new_page(viewport={"width": 1280, "height": 800, "deviceScaleFactor": 1.0})
              page.goto(vis_url, wait_until="load")
              _launched_browser = (pw, browser, page)
              print("[viser] headless Chromium connected via Playwright.")
          except Exception as e:
              print(f"[viser] Playwright launch skipped ({e}). You can open {vis_url} in a browser manually.")
      ...
      while not clients:
          clients = server.get_clients()
          if time.time() - t0 > 20 and _launched_browser is None:
              print(f"[viser] Waiting for a client… open {vis_url} in a browser.")
          time.sleep(0.1)

      client = list(clients.values())[0]
      print("[viser] Client connected:", client.client_id)


      # --- 3) 工具：K -> fov + aspect（Viser/Three.js使用fov与宽高比） ---
      def _k_to_fov_aspect(K: np.ndarray, H: int, W: int):
          fx, fy = float(K[0,0]), float(K[1,1])
          # 选用垂直视场（更稳定），你也可以改用水平 fov_x
          fov_y = 2.0 * np.arctan2(H, 2.0 * fy)  # 弧度
          aspect = float(W) / float(H)
          return float(fov_y), float(aspect)

      # --- 4) 渲染循环 ---
      interval_render   = interval
      render_output_dir = f"/data3/zihanwa3/_Robotics/_geo/differentiable-blocksworld/rendered_views/planar"
      os.makedirs(render_output_dir, exist_ok=True)

      render_output_dir = f"/data3/zihanwa3/_Robotics/_geo/differentiable-blocksworld/rendered_views/planar/{tgt_name}"
      os.makedirs(render_output_dir, exist_ok=True)
      rendered_frames = []

      # 建索引：frame_idx -> j
      idx_map = {fi: j for j, fi in enumerate(frame_indices)}

      # 可选：让交互无效，避免抖动
      client.camera.orthographic = False

      for frame_idx in range(0, num_frames, interval_render):
          j = idx_map.get(frame_idx)
          if j is None:
              continue
          ...
          # 相机位姿（世界坐标）
          cam_pos, cam_quat = _twc_to_cam_pose_world(T_wc)
          fov, _aspect = _k_to_fov_aspect(K, H, W)   # 还是用你的函数算 fov；aspect 交给 get_render 的宽高决定

          with client.atomic():
              client.camera.position = cam_pos              # (3,)
              client.camera.wxyz     = cam_quat             # (4,) ← 现在是真·四元数
              client.camera.fov      = float(fov)           # 弧度

              for ht, handles in hmr_mesh_handles.items():
                  want_show = gui_hmr_controls[ht].value  # 是否勾选显示该 HMR
                  for k, h in enumerate(handles):
                      h.visible = want_show and (k == j)
          # 请求一帧渲染（RGB, uint8, H×W×3）
          img = client.get_render(height=int(H), width=int(W))

          out_png = os.path.join(render_output_dir, f"frame_{frame_idx:05d}.png")
          cv2.imwrite(out_png, cv2.cvtColor(img, cv2.COLOR_RGB2BGR))
          rendered_frames.append(img)  # 存成数组以便合成视频

      print(f"Rendered {len(rendered_frames)} frames to {render_output_dir}")

      # --- 5) 可选：合成 MP4（需要 imageio-ffmpeg） ---
      with suppress(Exception):
          if len(rendered_frames) > 0:
              iio.imwrite(os.path.join(render_output_dir, "video.mp4"), rendered_frames, fps=30)
              print(f"Wrote video: {os.path.join(render_output_dir, 'video.mp4')}")
