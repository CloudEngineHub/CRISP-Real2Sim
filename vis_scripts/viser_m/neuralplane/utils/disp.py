...
def set_global_seed(seed: int = 42) -> None:
    ...
    # 1) pure-Python / stdlib RNG
    random.seed(seed)

    # 2) NumPy RNG
    np.random.seed(seed)

    # 3) PyTorch RNG (CPU & all GPUs)
    ...
    # 5) (optional) make hash-based ops repeatable across runs
...
def create_consistent_plane_visualization(image, plane_instances, n_planes):
    """
    Create visualization with consistent colors that match returned plane_colors.
    ...
    Returns:
        tuple: (overlay_image, plane_colors)
    """
    # Generate colors once
    palette = ColorPalette(num_of_colors=n_planes)
    plane_colors = np.stack([palette(i, type='int') for i in range(n_planes)], axis=0)
    
    # Create overlay using these exact colors
    overlay = image.copy()
    alpha = 0.5
    
    for i, mask in enumerate(plane_instances):
        if mask.sum() > 0:  # Only process non-empty masks
            # Create a colored version of the mask
            color_layer = np.zeros_like(image)
            color_layer[mask] = plane_colors[i]
            
            # Blend with original image
            overlay[mask] = (alpha * image[mask] + (1 - alpha) * color_layer[mask]).astype(np.uint8)
    
    return overlay, plane_colors
